...
*/
const ENDPOINT = 'n1ni6h03a0.execute-api.us-east-1.amazonaws.com/production';
const client = new AWS.ApiGatewayManagementApi({ endpoint: ENDPOINT });

const sendToOne = async (id: string, body: any) => {
  try {
...
const sendToAll = async (ids: Array<string>, body: any) => {
...
export const setPlayer = async(payload, meta) => {
  NAMES_DB[meta.connectionId] = payload.name;
  await sendToAll(Object.keys(NAMES_DB), { systemMessage: `${NAMES_DB[meta.connectionId]} player has joined the chat` })
}
